...
export const CATEGORIAS = {
  noticias:   { label: 'Noticias',   sub: 'La vuelta al mundo', color: 'var(--azul)' },
  columna:    { label: 'Columna',    sub: 'La Ventana',         color: 'var(--naranja)' },
  entrevista: { label: 'Entrevista', sub: 'El Pasajero',        color: 'var(--azul)' },
  informe:    { label: 'Informe',    sub: 'El Campamento',      color: '#1a1a2e' },
}
...
export default function NotaCard({ nota, featured = false }) {
      ...
      >
        ...
        <div style={{ position: 'relative', overflow: 'hidden', background: '#0f3aad', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {nota.foto_url
            ...
            : <div style={{ fontFamily: 'Pacifico, cursive', fontSize: 40, color: 'rgba(255,255,255,0.08)', textAlign: 'center', padding: 20, lineHeight: 1.3 }}>{cat.sub}</div>
          }
        </div>
      </div>
    )
  ...
  return (
    ...
    >
      <div style={{ height: 140, background: nota.foto_url ? 'transparent' : cat.color, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {nota.foto_url
          ? <img src={nota.foto_url} alt={nota.titulo} style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} />
          : <div style={{ fontFamily: 'Pacifico, cursive', fontSize: 18, color: 'rgba(255,255,255,0.15)', textAlign: 'center', padding: 16 }}>{cat.sub}</div>
        }
