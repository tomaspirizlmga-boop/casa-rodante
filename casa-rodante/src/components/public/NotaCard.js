        ...
        <div style={{ padding: '28px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>
              {nota.autores?.join(', ')} · {new Date(nota.created_at).toLocaleDateString('es-UY', { day: 'numeric', month: 'short', year: 'numeric' })}
            </span>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '8px 14px', borderTop: '1px solid var(--crema-dark)', display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#aaa' }}>
        <span>{nota.autores?.join(', ')} · {new Date(nota.created_at).toLocaleDateString('es-UY', { day: 'numeric', month: 'short' })}</span>
