...
const emptyForm = { titulo: '', resumen: '', cuerpo: '', categoria: 'entrevista', autores: [], publicada: false }
...
export default function Redaccion({ session }) {
  ...
  const handleEdit = (nota) => {
    setForm({ titulo: nota.titulo, resumen: nota.resumen || '', cuerpo: nota.cuerpo || '', categoria: nota.categoria, autores: nota.autores || [], publicada: nota.publicada })
    setEditId(nota.id)
    setFotoFile(null)
    setAutorInput('')
    setSeccion('nueva')
  ...
  const handleAutorKeyDown = (e) => {
    if (e.key === 'Enter' && autorInput.trim()) {
      e.preventDefault()
      if (!form.autores.includes(autorInput.trim())) {
        setForm(f => ({ ...f, autores: [...f.autores, autorInput.trim()] }))
      }
      setAutorInput('')
    }
  }

  const removeAutor = (name) => {
    setForm(f => ({ ...f, autores: f.autores.filter(a => a !== name) }))
  }
      ...
      <aside style={{ width: 200, background: 'var(--sidebar)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div style={{ padding: '12px 0', flex: 1 }}>
          ...
          {[
            ...
            <button
              key={item.id}
              onClick={() => { setSeccion(item.id); if (item.id === 'nueva') { setForm(emptyForm); setEditId(null); setAutorInput('') } }}
              style={{
        ...
        {seccion === 'nueva' && (
          <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 32px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 5 }}>Autor/es</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, border: '1px solid var(--borde)', borderRadius: 8, padding: '8px 10px', background: '#fff', minHeight: 40, alignItems: 'center' }}>
                  {form.autores.map(a => (
                    <span key={a} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'var(--azul)', color: '#fff', fontSize: 12, padding: '3px 8px', borderRadius: 12 }}>
                      {a}
                      <button type="button" onClick={() => removeAutor(a)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: 13, lineHeight: 1, padding: 0 }}>×</button>
                    </span>
                  ))}
                  <input
                    value={autorInput}
                    ...
                    placeholder={form.autores.length === 0 ? 'Escribí un nombre y Enter' : ''}
        ...
        {seccion === 'notas' && (
          <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={() => { setForm(emptyForm); setEditId(null); setAutorInput(''); setSeccion('nueva') }}
                ...
                <div key={nota.id} style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--borde)', padding: '12px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#888' }}>
                      {CATEGORIAS.find(c => c.val === nota.categoria)?.label} · {new Date(nota.created_at).toLocaleDateString('es-UY')} · {nota.autores?.join(', ')}
                    </div>
                  </div>
